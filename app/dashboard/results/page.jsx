...
function ResultsContent() {
  ...
  const [{ featureWinners, overallWinner, isTie, itemScores }, setAnalysis] =
    useState({
      featureWinners: {},
      overallWinner: null,
      isTie: false,
  ...
  const analyzeFeatureWinners = (table, items) => {
    ...
    table.forEach((row) => {
      ...
      if (
        featureName.includes("feature") ||
        featureName.includes("name") ||
        featureName.includes("color")
      )
        return;
      ...
      if (numericValues.every((v) => v !== null)) {
        ...
        const higherIsBetter = !["price", "weight", "cost", "consumption"].some(
          (term) => featureName.includes(term),
        );
        ...
        if (bestIndices.length === 1) {
          ...
          const itemName =
            typeof items[bestIndices[0]] === "object"
        ...
        if (uniqueValues.size === 1) {
          ...
          const bestIndex = values.reduce(
            (best, curr, idx) =>
              curr.length > values[best].length ? idx : best,
            0,
          );
          featureWinners[featureName] = bestIndex;
          const itemName =
            typeof items[bestIndex] === "object"
      ...
      try {
        if (comparisonId) {
          ...
          if (result && result[0]) {
            ...
            const analysis = analyzeFeatureWinners(
              result[0].result.table,
              result[0].result.items,
            );
  ...
  const handleDownload = () => {
    ...
    let csvContent =
      "Feature," +
      comparisonData.items.map((item) => `"${item.name}"`).join(",") +
      "\n";
                ...
                >
                  ...
                  <div className="mt-3 flex flex-wrap gap-1">
                    <div className="text-sm bg-green-100 text-green-800 px-2 py-1 rounded flex items-center">
                      <ThumbsUp className="h-3 w-3 mr-1" /> {item.pros.length}{" "}
                      Pros
                    </div>
                    <div className="text-sm bg-red-100 text-red-800 px-2 py-1 rounded flex items-center">
                      <ThumbsDown className="h-3 w-3 mr-1" /> {item.cons.length}{" "}
                      Cons
                    </div>
                  </div>
                </div>
              ...
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-green-700">
                    <span className="font-semibold">{overallWinner}</span> -
              ...
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-blue-700">
                    The products are evenly matched. Choose based on your
                    specific needs.
                  </p>
                </div>
              </div>
          ...
          <section className="mb-8">
            ...
            <div className="mb-6 bg-white rounded-lg shadow p-4">
              <div className="flex flex-wrap gap-2">
                ...
                {activeFeature === null &&
                  setActiveFeature(Object.keys(featureCategories)[0])}
              </div>
            </div>
            ...
            <div className="space-y-4">
              {activeFeature &&
                featureCategories[activeFeature]?.map((row, rowIdx) => {
                  const cells = parseTableRow(row);
                    ...
                    >
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 p-4">
                        {cells.slice(1).map((cell, cellIdx) => {
                          ...
                          const itemName =
                            comparisonData.items[cellIdx]?.name ||
                            `Item ${cellIdx + 1}`;
                  ...
                  return (
                    ...
                    >
                      ...
                      {cells.slice(1).map((cell, j) => {
                        const isWinner =
                          featureWinner === j || featureWinner === "both";
              ...
              {overallWinner ? (
                ...
                <div className="mt-6 p-4 bg-blue-50 rounded-lg border border-blue-200 flex items-start">
                  ...
                  <div>
                    ...
                    <p className="text-blue-700">
                      The products are very evenly matched. Choose based on your
                      specific needs.
                    </p>
                  </div>
                </div>
